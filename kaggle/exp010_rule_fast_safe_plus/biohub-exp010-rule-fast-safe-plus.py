...
class Config:
    ...
    # -- 031 conservative post-link division recovery --
    safe_divisions: bool = False
    safe_division_max_um: float = 5.0
    safe_division_sibling_max_um: float = 8.0
    safe_division_existing_child_max_um: float = 8.5
    safe_division_frame_frac_cap: float = 0.012
    safe_division_global_frac_cap: float = 0.006
...
def run_one(zarr_path: str, cfg: Config, t_limit: int | None = None) -> io.TrackGraph:
    ...
    if cfg.safe_divisions:
        g = add_safe_divisions_postlink(
            frames, g,
            max_div_um=cfg.safe_division_max_um,
...
def add_safe_divisions_postlink(frames: list, g: io.TrackGraph,
                                max_div_um: float = 5.0,
                                sibling_max_um: float = 8.0,
                                existing_child_max_um: float = 8.5,
                                frame_frac_cap: float = 0.012,
                                global_frac_cap: float = 0.006) -> io.TrackGraph:
    """Very conservative second-daughter recovery.

    The 0.835 rule-based baseline has strong nodes and normal links but predicts no
    explicit mitosis unless the simple linker is allowed to do so.  This post-pass
    adds only a *second* child edge for a source that already has one next-frame
    child, and only when there is an otherwise-unlinked detection in t+1 that is
    physically close to the source and geometrically plausible as a sibling.

    It never changes nodes or existing edges.  It also keeps caps low so that a
    bad dataset cannot flood the submission with false divisions.
    """
    if g.n_nodes == 0 or g.n_edges == 0:
        g.meta = dict(g.meta or {})
        g.meta["safe_divisions_added"] = 0
        return g

    coords = g.coords_by_id()
    out = {}
    inc = {}
    for s, t in g.edges:
    ...
    # ids by frame from graph nodes
    ids_by_t = {}
    for nid, (tt, z, y, x) in coords.items():
        ids_by_t.setdefault(int(tt), []).append(int(nid))

    existing_edges = set((int(s), int(t)) for s, t in g.edges)
    added = []
    used_new_targets = set()
    global_cap = max(1, int(max(1, g.n_edges) * global_frac_cap))

    for tt in sorted(ids_by_t.keys()):
        if tt + 1 not in ids_by_t:
            continue
        src_ids = [nid for nid in ids_by_t[tt] if len(out.get(nid, [])) == 1]
        if not src_ids:
            continue
        # second daughter should not already have an incoming edge
        cand_ids = [nid for nid in ids_by_t[tt + 1] if nid not in inc and nid not in used_new_targets]
        if not cand_ids:
            continue

        frame_cap = max(1, int(len(src_ids) * frame_frac_cap))
        proposals = []
        cand_pos = np.array([[coords[c][1], coords[c][2], coords[c][3]] for c in cand_ids], dtype=float) * SCALE

        for sid in src_ids:
            child = out[sid][0]
            # only true consecutive links for division geometry
            if coords.get(child, (None,))[0] != tt + 1:
                continue
            sp = np.array([coords[sid][1], coords[sid][2], coords[sid][3]], dtype=float) * SCALE
            cp = np.array([coords[child][1], coords[child][2], coords[child][3]], dtype=float) * SCALE
            d_child = float(np.linalg.norm(cp - sp))
            if d_child > existing_child_max_um:
                continue
            d_parent = np.sqrt(((cand_pos - sp[None, :]) ** 2).sum(axis=1))
            d_sibling = np.sqrt(((cand_pos - cp[None, :]) ** 2).sum(axis=1))
            ok = (d_parent <= max_div_um) & (d_sibling <= sibling_max_um)
            if not np.any(ok):
                continue
            idxs = np.where(ok)[0]
            # prefer closest-to-parent, but lightly penalize duplicate-like sibling closeness
            scores = d_parent[idxs] + 0.15 * d_sibling[idxs]
            best_local = int(idxs[int(np.argmin(scores))])
            target = cand_ids[best_local]
            if (sid, target) in existing_edges:
                continue
            proposals.append((float(scores.min()), sid, target, float(d_parent[best_local]), float(d_sibling[best_local])))

        if not proposals:
            continue
        proposals.sort(key=lambda x: x[0])
        added_this_frame = 0
        for _, sid, target, dp, ds in proposals:
            if len(added) >= global_cap or added_this_frame >= frame_cap:
                break
            if target in used_new_targets or target in inc:
                continue
            added.append((sid, target))
            used_new_targets.add(target)
            added_this_frame += 1

    if not added:
        g.meta = dict(g.meta or {})
        g.meta["safe_divisions_added"] = 0
        g.meta["safe_divisions_global_cap"] = global_cap
        return g

    new_edges = np.concatenate([g.edges, np.array(added, dtype=np.int64).reshape(-1, 2)], axis=0)
    meta = dict(g.meta or {})
    meta["safe_divisions_added"] = int(len(added))
    meta["safe_divisions_global_cap"] = int(global_cap)
    return io.TrackGraph(node_t=g.node_t, node_z=g.node_z, node_y=g.node_y,
                         node_x=g.node_x, node_ids=g.node_ids,
                         edges=new_edges, meta=meta)
...
CONFIG_OVERRIDE = {
    # Start from the acquired LB 0.835 rule-based baseline.
    'detector': 'blob',
    ...
    # Fast 034 score-up axis: keep the exp006 runtime-safe pipeline, with
    # slightly wider second-daughter geometry than the 0.839 anchor.
    # Keep built-in allow_divisions OFF; the custom post-pass is much more guarded.
    'allow_divisions': False,
    'safe_divisions': True,
    'safe_division_max_um': 5.35,
    'safe_division_sibling_max_um': 8.5,
    'safe_division_existing_child_max_um': 9.0,
    'safe_division_frame_frac_cap': 0.014,
    'safe_division_global_frac_cap': 0.008,
}
...
def main():
    ...
    for i, name in enumerate(names):
        ...
        div_added = int((g.meta or {}).get("safe_divisions_added", 0))
        print(f"[{i+1}/{len(names)}] {name}: nodes={g.n_nodes} edges={g.n_edges} "
              f"safe_divisions_added={div_added} ({time.time()-t0:.1f}s)", flush=True)
    out = "submission.csv"
    write_submission(all_rows, out)
    print(f"wrote {out}: {len(all_rows)} rows in {time.time()-t0:.1f}s", flush=True)
    summary = {
        "experiment": "exp010_rule_fast_safe_plus",
        "base": "lb0835_rule_based",
        "change": "fast_safe_division_plus",
